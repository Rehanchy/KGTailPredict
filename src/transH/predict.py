...
model = TransH(entity_pre_emb["word2index"],
                relation_pre_emb["rel2idx"],
                embed_dimension = 100)

checkpoint = torch.load("./output/transHtrained.bin", map_location="cpu")
...
with open("./submit/result5.txt", "w", encoding="utf-8") as fw:
